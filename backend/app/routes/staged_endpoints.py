...
from app.models import PipelineStage, GenerateMoreRequest, ProceedToStageRequest, Epic, UserStory, AcceptanceCriterion, FunctionalTest, GherkinScenario
...
async def proceed_to_stage(job_id: str, request: ProceedToStageRequest, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        if stage_already_completed:
            ...
            return {"job_id": job_id, "stage": stage, "status": "already_completed"}

        job.advance_stage(stage)

        if not job.job_dir:
            raise HTTPException(status_code=400, detail="BRD file not found")
        uploaded_files = list(Path(job.job_dir).glob("brd.*"))
        ...
        return {"job_id": job_id, "stage": stage, "status": "started"}
...
async def generate_more(job_id: str, request: GenerateMoreRequest, background_tasks: BackgroundTasks):
    ...
    try:
            ...
            try:
                ...
                if request.stage == PipelineStage.EPICS:
                    ...
                    if request.context_ids:
                        ...
                        log_info("Filtering stories for Functional Tests", job_id=job_id, filtered=len(stories_to_use))
                    ...
                    if request.context_ids:
                        stories_to_use = [s for s in job.results.user_stories if s.id in request.context_ids]
                        log_info("Filtering stories for Gherkin generation", job_id=job_id, filtered=len(stories_to_use))

                    functional_tests_to_use = job.results.functional_tests
                    if request.context_ids:
                        functional_tests_to_use = [ft for ft in job.results.functional_tests if ft.story_id in request.context_ids]
        ...
        return {"job_id": job_id, "stage": request.stage, "status": "generating"}
